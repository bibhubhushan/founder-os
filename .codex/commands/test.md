...
- Find existing tests with **Glob** (`**/*.test.*`, `**/*.spec.*`, `**/test_*`, `**/*_test.*`)
- Read `memory/preferences.md` for preferred test patterns

### 2. Determine What to Test
If `$ARGUMENTS` specifies a file or feature, test that.
Otherwise:
...
- Read the source file with **Read**
